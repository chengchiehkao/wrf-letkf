...
real(kind=8),dimension(1) :: dummy_pressureV
...
#endif
do isn = 1+boundaryOffset,domain(1)%size_southToNorth_stag-boundaryOffset
do iwe = 1+boundaryOffset,domain(1)%size_westToEast-boundaryOffset

    call locateAsIndex2d( domain(1)%lon(:,:) , domain(1)%lat(:,:) , domain(1)%size_westToEast , domain(1)%size_southToNorth , &
                          domain(1)%lon_v(iwe,isn) , domain(1)%lat_v(iwe,isn) , &
    ...
    do id = 1,domainSize
        ...
        do iz = 1,domain(id)%size_bottomToTop
            call interp2d( domain(id)%lon(targetRankOneIndex:targetRankOneIndex+1,targetRankTwoIndex:targetRankTwoIndex+1) , &
                           ...
                           2 , 2 , &
                           (/domain(id)%lon_v(iwe,isn)/) , (/domain(id)%lat_v(iwe,isn)/) , dummy_pressureV(:) , &
                           1 , &
                           1 , invalidValue )
            domain(id)%pressure_v(iwe,isn,iz) = dummy_pressureV(1)
        enddo
