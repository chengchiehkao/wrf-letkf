...
do iwe = 1+systemParameters_boundaryWidth,domain_mean%size_westToEast_stag-systemParameters_boundaryWidth

    if ( obsListOfEachGrid(iwe,isn,iz)%vectorSize .eq. 0 ) then  ! no observation means won't update.
        forall(iens=1:ensembleSize) analysis(iens)%u(iwe,isn,iz) = background(iens)%u(iwe,isn,iz)
        cycle
    else
        ...
        do iens = 1,ensembleSize
            xb_mean(1,1) = xb_mean(1,1) + background(iens)%u(iwe,isn,iz)
        enddo
        xb_mean(:,:) = xb_mean(:,:) / real(ensembleSize,8)

        forall(iens=1:ensembleSize) xb_pert(1,iens) = background(iens)%u(iwe,isn,iz) - xb_mean(1,1)
        ...
        do iList = 1,obsListOfEachGrid(iwe,isn,iz)%vectorSize
            if ( obs%obs( obsListOfEachGrid(iwe,isn,iz)%vector(iList) )%available ) then
                ...
                dh = greatCircleDistance( &
                  (/domain_mean%lon_u(iwe,isn),domain_mean%lat_u(iwe,isn)/) , &
                  (/obs%obs( obsListOfEachGrid(iwe,isn,iz)%vector(iList) )%lon,obs%obs( obsListOfEachGrid(iwe,isn,iz)%vector(iList) )%lat/) &
                  )
                dz = dabs( dlog(domain_mean%pressure_u(iwe,isn,iz)) - dlog(obs%obs( obsListOfEachGrid(iwe,isn,iz)%vector(iList) )%z) )
        ...
        forall(iens=1:ensembleSize) analysis(iens)%u(iwe,isn,iz) = xa_mean(1,1) + xa_pert(1,iens)
